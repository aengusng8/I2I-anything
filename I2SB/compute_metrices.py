...
def find_recon_imgs_pts(opt, log):
    ...
    assert (
        len(recon_imgs_pts) > 0
    ), f"Found 0 file that matches '{str(sample_dir)}/recon_*.pt'!"
...
def log_metrices(opt):
    ...
    log.info(
        f"Found {len(recon_imgs_pts)} pt files={[pt.name for pt in recon_imgs_pts]}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="the checkpoint name for which we wish to compute metrices",
    )
    parser.add_argument(
        "--mode",
        type=str,
