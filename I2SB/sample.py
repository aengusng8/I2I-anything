...
def build_subset_per_gpu(opt, dataset, log):
    ...
    log.info(
        f"[Dataset] Add {n_dump} data to the end to be devided by {n_gpu=}. Total length={len(total_idx)}!"
    )
    ...
    log.info(
        f"[Dataset] Built subset for gpu={opt.global_rank}! Now size={len(subset)}!"
    )
    return subset
...
def build_partition(opt, full_dataset, log):
    ...
    log.info(
        f"[Dataset] Built partition={opt.partition}, {start_idx=}, {end_idx=}! Now size={len(subset)}!"
    )
    return subset
...
def build_val_dataset(opt, log, corrupt_type):
    if "sr4x" in corrupt_type:
        ...
        val_dataset = imagenet.InpaintingVal10kSubset(
            opt, log, mask
        )  # subset 10k val + mask
        ...
        val_dataset = MixtureCorruptDatasetVal(
            opt, val_dataset
        )  # subset 10k val + mixture
    else:
...
def get_recon_imgs_fn(opt, nfe):
    sample_dir = (
        RESULT_DIR
        / opt.ckpt
        / "samples_nfe{}{}".format(nfe, "_clip" if opt.clip_denoise else "")
    )
...
def compute_batch(ckpt_opt, corrupt_type, corrupt_method, out):
    if "inpaint" in corrupt_type:
        ...
        corrupt_img = clean_img * (1.0 - mask) + mask
        x1 = clean_img * (1.0 - mask) + mask * torch.randn_like(clean_img)
    elif corrupt_type == "mixture":
...
def main(opt):
    ...
    if opt.use_fp16:
        ...
        runner.ema = ExponentialMovingAverage(
            runner.net.parameters(), decay=0.99
    ...
    for loader_itr, out in enumerate(val_loader):
        corrupt_img, x1, mask, cond, y = compute_batch(
            ckpt_opt, corrupt_type, corrupt_method, out
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="e.g., '0_4' means the first 25% of the dataset",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="the checkpoint name from which we wish to sample",
    )
    ...
    parser.add_argument(
        "--use-fp16",
        action="store_true",
        help="use fp16 network weight for faster sampling",
    )
    ...
    if opt.distributed:
        ...
        for rank in range(size):
            ...
            p = Process(
                target=dist_util.init_processes,
                args=(global_rank, global_size, main, opt),
            )
            p.start()
            processes.append(p)
