...
def create_training_options():
    ...
    parser.add_argument(
        "--ckpt", type=str, default=None, help="resumed checkpoint name"
    )
    parser.add_argument(
        "--gpu",
        type=int,
        default=None,
        help="set only if you wish to run on a particular device",
    )
    parser.add_argument(
        "--n-gpu-per-node", type=int, default=1, help="number of gpu on each node"
    )
    parser.add_argument(
        "--master-address", type=str, default="localhost", help="address for master"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-4,
        help="sigma start time in network parametrization",
    )
    parser.add_argument(
        "--T", type=float, default=1.0, help="sigma end time in network parametrization"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.3,
        help="max diffusion for the diffusion model",
    )
    ...
    parser.add_argument(
        "--clip-denoise",
        action="store_true",
        help="clamp predicted image to [-1,1] at each",
    )
    ...
    parser.add_argument(
        "--cond-x1",
        action="store_true",
        help="conditional the network on degraded images",
    )
    parser.add_argument(
        "--add-x1-noise", action="store_true", help="add noise to conditional network"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="accumulate gradient over microbatch until full batch-size",
    )
    parser.add_argument(
        "--num-itr", type=int, default=1000000, help="training iteration"
    )
    parser.add_argument("--lr", type=float, default=5e-5, help="learning rate")
    parser.add_argument(
        "--lr-gamma", type=float, default=0.99, help="learning rate decay ratio"
    )
    parser.add_argument(
        "--lr-step", type=int, default=1000, help="learning rate decay step size"
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=".log",
        help="path to log std outputs and writer data",
    )
    parser.add_argument(
        "--log-writer",
        type=str,
        default=None,
        help="log writer: can be tensorbard, wandb, or None",
    )
    parser.add_argument(
        "--wandb-api-key",
        type=str,
        default=None,
        help="unique API key of your W&B account; see https://wandb.ai/authorize",
    )
    parser.add_argument(
        "--wandb-user", type=str, default=None, help="user name of your W&B account"
    )
    ...
    # ========= auto assert =========
    assert (
        opt.batch_size % opt.microbatch == 0
    ), f"{opt.batch_size=} is not dividable by {opt.microbatch}!"
    return opt
...
if __name__ == "__main__":
    ...
    if opt.distributed:
        ...
        for rank in range(size):
            ...
            p = Process(
                target=init_processes, args=(global_rank, global_size, main, opt)
            )
            p.start()
            processes.append(p)
