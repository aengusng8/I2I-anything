...
class ColorDataset(Dataset):
    ...
    LAB_OFFSET = np.array([[50.0, 0.0, 0.0]]).astype(np.float32)
    LAB_DENOMINATOR = np.array([[50.0, 128.0, 128.0]]).astype(np.float32)
    LAB_UPPER_BOUND = np.array([[100.0, 127.0, 127.0]]).astype(np.float32)
    LAB_LOWER_BOUND = np.array([[0.0, -128.0, -128.0]]).astype(np.float32)
...
def load_color_data_for_training(batch_size, image_path, use_lab_space, logger):
    dataset = ColorDataset(image_path=image_path, use_lab_space=use_lab_space)
    logger.log(f"dataset length: {len(dataset)}, {dataset.points.shape}")
    logger.log(
        f"dataset points: {dataset.points}, max: {dataset.points.max(0)}, min: {dataset.points.min(0)}"
    )
    loader = DataLoader(dataset, batch_size=batch_size, num_workers=1, drop_last=False)
    while True:
        yield from loader
