...
def main():
    ...
    data = load_2d_data(
        n_samples=90000, batch_size=args.batch_size, shape=shape_s, training=False
    )
    ...
    logger.log(
        f"computing the max L2 distance between original data points and round-trip:"
    )
...
def create_argparser():
    defaults = dict(num_samples=90000, batch_size=30000, model_path="")
    defaults.update(model_and_diffusion_defaults_2d())
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--source", type=int, default=0, help="Source synthetic dataset."
    )
    parser.add_argument(
        "--target", type=int, default=1, help="Target synthetic dataset."
    )
